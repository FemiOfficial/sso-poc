...
func (c *AppController) AddAppIdentityProvider(ctx *gin.Context) {
	message, err, statusCode := c.appService.AddAppIdentityProvider(ctx)
	if err != nil {
		fmt.Println("Error adding app identity provider: ", err, statusCode)
		message := err.Error()
		ctx.JSON(*statusCode, utils.GenericApiResponse(*statusCode, &message, nil))
		return
	}
